...
해설 파일 끝에 항목을 덧붙일 때 **마지막 기존 항목에는 콤마가 없다.** 콤마 없이 이어 붙이면 JS가 앞 항목의 문자열과 새 항목의 ID·본문을 한 덩어리로 읽어 객체가 조용히 깨진다 — 에러도 안 나고 해설만 사라진다. 붙이기 전에 콤마를 넣고, 아래 로더 검증으로 항목 수가 늘었는지 확인한다.
